...
static int
hangul_jaso_to_string(ucschar cho_, ucschar jung_, ucschar jong_, ucschar mode_,
		      ucschar *buf, int len)
{
    ucschar ch = 0;
    int n = 0;

    ucschar cho = hangul_vkeycode_to_keycode(cho_);
    ucschar jung = hangul_vkeycode_to_keycode(jung_);
    ucschar jong = hangul_vkeycode_to_keycode(jong_);
    ucschar mode = hangul_vkeycode_to_keycode(mode_);
    if (jong == 0 && hangul_is_jongseong(mode)) {
        jong = mode;
        mode = 0;
    }
...
static bool
hangul_buffer_backspace_gureum(HangulInputContext *hic)
{
    HangulBuffer *buffer = &hic->buffer;
    if (buffer->index >= 0) {
        hangul_buffer_pop(buffer);
        hangul_buffer_pop(buffer);
        hangul_buffer_pop(buffer);
        hangul_buffer_pop(buffer);
        hangul_buffer_pop(buffer);
        ...
        if (buffer->index >= 0) {
            buffer->choseong = buffer->stack[buffer->index-4];
            buffer->jungseong = buffer->stack[buffer->index-3];
            buffer->jongseong = buffer->stack[buffer->index-2];
            buffer->state = buffer->stack[buffer->index-1];
        } else {
            buffer->choseong = 0;
            buffer->jungseong = 0;
            buffer->jongseong = 0;
...
static inline void
hangul_ic_push_gureum(HangulInputContext *hic, ucschar ch)
{
    HangulBuffer *buffer = &hic->buffer;
    buffer->stack[++buffer->index] = buffer->choseong;
    buffer->stack[++buffer->index] = buffer->jungseong;
    buffer->stack[++buffer->index] = buffer->jongseong;
    buffer->stack[++buffer->index] = buffer->state;
    buffer->stack[++buffer->index] = ch;
}
...
hangul_ic_process_gureum(HangulInputContext *hic, ucschar ch)
{
    ...
    // 최종 순아래에서 초성/중성 없고 ㅚ ㅟ 주어지면 특수문자로 처리
    if (hic->buffer.choseong == 0 && hic->buffer.jungseong == 0 && hic->buffer.state == 0) {
        if (hangul_vkeycode_to_keycode(ch) == 0x1171) {
            // ㅟ -> '
            ch = 0x27;
        } else if (hangul_vkeycode_to_keycode(ch) == 0x116c) {
            // ㅚ -> /
            ch = 0x2f;
        }
    }
    ...
    if (hangul_is_choseong(ch)) {
        if (hangul_is_jungseong(hic->buffer.state)) {
            ...
            hic->buffer.state = 0;
            hangul_ic_save_commit_string(hic);
            hic->buffer.jungseong = jungseong;
            hangul_ic_push_gureum(hic, jungseong);
            hic->buffer.choseong = ch;
            hangul_ic_push_gureum(hic, ch);
        } else if (hic->buffer.choseong == 0) {
            if (hic->buffer.jongseong && hic->buffer.state == mode_key) {
                // ㅏㄻㅅ[  f[[n
                hic->buffer.state = 0;
                hangul_ic_flush_internal(hic);
                hic->buffer.state = mode_key;
                hangul_ic_push_gureum(hic, mode_key);
            }
            // 간 k, fk, sfk
            hic->buffer.choseong = ch;
            hangul_ic_push_gureum(hic, ch);
            ...
            if (choseong) {
                // 까 jk fjk
                hic->buffer.choseong = choseong;
                hangul_ic_push_gureum(hic, choseong);
            } else if (hangul_is_jungseong(hic->buffer.state) || hic->buffer.state == mode_key) {
                // 까ㅁ fkji
                ucschar jungseong = hic->buffer.state;
                hic->buffer.state = 0;
                hangul_ic_save_commit_string(hic);
                ...
                hic->buffer.choseong = ch;
                hangul_ic_push_gureum(hic, ch);
            } else if (hangul_is_jongseong(hic->buffer.state)) {
                if (hic->buffer.jongseong == 0) {
                    // 많이
                    hic->buffer.jongseong = hic->buffer.state;
                    hic->buffer.state = 0;
                    hangul_ic_save_commit_string(hic);
                } else {
                    // possible?
                    ucschar jongseong = hic->buffer.state;
                    hic->buffer.state = 0;
                    hangul_ic_save_commit_string(hic);
        ...
        if (hic->buffer.state) {
            if (hic->buffer.state == mode_key) {
                ...
                if (jongseong) {
                    ...
                    hic->buffer.state = 0;
                    hic->buffer.jongseong = jongseong;
                    hangul_ic_push_gureum(hic, jongseong);
                } else if (hangul_is_jongseong(hic->buffer.state) && hic->buffer.jongseong == 0) {
                    // ㅏㄻ[ f[[
                    hic->buffer.jongseong = hic->buffer.state;
                    hic->buffer.state = ch;
                    hangul_ic_push_gureum(hic, ch);
            ...
            hic->buffer.state = state;
            hangul_ic_push_gureum(hic, state);
        } else {
            ucschar jongseong = 0;
            if (hic->buffer.jongseong) {
                jongseong = hangul_ic_combine(hic, hic->buffer.jongseong, ch);
        ...
        if (hic->buffer.jungseong == 0) {
            if (hic->buffer.state == mode_key) {
                // n[[f
                ucschar jongseong = hangul_ic_combine(hic, ch, mode_key);
                if (jongseong) {
                    // 삶 n[f
                    hic->buffer.state = jongseong;
                }
            }
        ...
        if (hic->buffer.jongseong == 0) {
            if (hangul_is_jungseong(hic->buffer.state)) {
                ...
                ucschar jongseong = 0;
                if (hic->buffer.state) {
                    jongseong = hangul_ic_combine(hic, ch, mode_key);
                }
                if (jongseong) {
                    // 얹 jt[e j[st
                    hic->buffer.state = 0;
...
hangul_ic_backspace(HangulInputContext *hic)
{
    ...
    int type = hangul_keyboard_get_type(hic->keyboard);
    if (type == HANGUL_KEYBOARD_TYPE_GUREUM) {
        ret = hangul_buffer_backspace_gureum(hic);
    } else {
